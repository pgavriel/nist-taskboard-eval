...
def processTBImage(image):
    ...
    cv2.imshow('processed',tb)
    return tb
    # tb = cv2.dilate(tb,kernel,iterations = 1)
    # tb = cv2.erode(tb,kernel,iterations = 1)
    pass
...
def main(DIR,eval_image):
    ...
    emptyboard2 = processTBImage(emptyboard)
    emp_gray = cv2.cvtColor(emptyboard, cv2.COLOR_BGR2GRAY)
    contours, hierarchy = cv2.findContours(emp_gray,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    emptyboard = cv2.drawContours(emptyboard, contours, -1, (0,255,0), 3)
    print("CONTOURS", len(contours),"\n",contours)
    cv2.imshow('cont',emptyboard)
    ...
    # #emptyboard_g = cv2.morphologyEx(emptyboard_g, cv2.MORPH_OPEN, kernel)
    # emptyboard_g = cv2.dilate(emptyboard_g,kernel,iterations = 1)
    # emptyboard_g = cv2.erode(emptyboard_g,kernel,iterations = 1)


    #emptyboard_g3 = cv2.cvtColor(emptyboard_g,cv2.COLOR_GRAY2BGR)
    fullboard = cv2.imread(os.path.join(DIR,'20/20-009-Warped.png'))
    fullboard2 = processTBImage(fullboard)
    ...
    # # fullboard_g = cv2.morphologyEx(fullboard_g, cv2.MORPH_OPEN, kernel)
    # fullboard_g = cv2.dilate(fullboard_g,kernel,iterations = 1)
    # fullboard_g = cv2.erode(fullboard_g,kernel,iterations = 1)
    # cv2.imshow('emptyboard',emptyboard_g)
    # cv2.imshow('full',fullboard_g)
    ad = cv2.absdiff(emptyboard2,fullboard2)
    cv2.imshow('ad',ad)
